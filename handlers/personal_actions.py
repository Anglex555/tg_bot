...
def main_menu():
    ...
    item5 = types.KeyboardButton("Загрузить ДЗ на дату")
    menu.add(item1, item2, item3, item4, item5)
    return menu
...
async def cal(message):
    global users
    for user_i in users:
        if(user_i.id == message.from_user.id):
            user = user_i
    result, key, step = DetailedTelegramCalendar(locale='ru').process(message.data)
    if not result and key:
        if str(LSTEP[step]) == "year":
            await message.bot.edit_message_text("Выберите год",
                            message.message.chat.id,
                            message.message.message_id,
                            reply_markup=key)
        if str(LSTEP[step]) == "month":
            await message.bot.edit_message_text("Выберите месяц",
                            message.message.chat.id,
                            message.message.message_id,
                            reply_markup=key)
        if str(LSTEP[step]) == "day":
            await message.bot.edit_message_text("Выберите день",
                            message.message.chat.id,
                            message.message.message_id,
                            reply_markup=key)
    elif result:
        user.date_hw_cal = str(result)
        await message.bot.edit_message_text(f"{result}",
                            message.message.chat.id,
                            message.message.message_id)
        if user.stage_hw == 1:
            
            date_hf = user.date_hw_cal
            user.date_hw = date_hf[8] + date_hf[9] + "." + date_hf[5] + date_hf[6] + "." + date_hf[0] + date_hf[1] + date_hf[2] +date_hf[3]
            ...
            await message.bot.send_message(message.message.chat.id, 'Выберите предмет'.format(message.from_user), reply_markup = menu)
        if user.stage_get_hw_gf == 1:
            ...
            user.stage_get_hw = 0
            isodate = int(date_hf[8] + date_hf[9])
            isomonth = int(date_hf[5] + date_hf[6])
            isoyear = int(date_hf[0] + date_hf[1] + date_hf[2] +date_hf[3])
                ...
                for r in records:
                    ...
                    if(cmd != '' and r[0] > 0):
                        ...
                        if (date_out == isodate and month_out == isomonth and year_out == isoyear):
                            if ('file' in homework_out):
                                if (user.subject_out not in imgs):
                                    imgs.append(user.subject_out)
                                imgs.append(homework_out)
                            else:
                                answer += "<b>" + (user.subject_out) + "</b>\n"
                                answer += f" - {homework_out}\n\n"

                await message.bot.send_message(message.message.chat.id, answer.format(message.from_user), reply_markup = menu)
                for img in imgs:
                    if('file' in img):
                        photo = open('files/photos/' + str(img), 'rb')
                        await message.bot.send_photo(message.message.chat.id, photo = photo)
                    else:
                        await message.bot.send_message(message.message.chat.id, ("<b>" + str(img) + "</b>:\n ").format(message.from_user))
            else:
                await message.bot.send_message(message.message.chat.id, 'Записей не обнаружено!'.format(message.from_user), reply_markup = menu)
...
async def start(message):
    ...
    photo_i += 1
    user.imgs.append('file_' + str(photo_i) + '.jpg')
    await photo.download('files/photos/file_' + str(photo_i) + '.jpg')

    if user.stage_hw == 3:
        for img in user.imgs:
            homework_db = img

            homework_db = user.date_hw + user.subject + "%&%" + homework_db
            first = '...'
            last = '...'
            if(str(message.from_user.first_name) != 'None'):
...
async def start(message):
    ...
    if message.text == 'anarchy' or message.text == 'Anarchy':
        ...
        photo = open('files/timetable.png', 'rb')
        await message.bot.send_photo(message.chat.id, photo = photo)

    elif message.text == 'Загрузить ДЗ на дату':
        if(message.from_user.id in administrators or anarchy):
            ...
            # message.bot.send_message(message.chat.id, f"Select {LSTEP[step]}", reply_markup=calendar)
        else:
        ...
        if(message.from_user.id in administrators or anarchy):
            ...
            today_date = check_date_i(today_date, now_month, num_day)
            if (int(today_date) < 5):
                num_day += 1
        ...
        user.stage_hw = 3
        await message.bot.send_message(message.chat.id, 'Напишите задание или отправте фото'.format(message.from_user))
        ...
        if(len(records)):
            ...
            for img in imgs:
                if('file' in img):
                    ...
                    await message.bot.send_message(message.chat.id, ("<b>" + str(img) + "</b>:\n ").format(message.from_user)) 
        else:
            await message.reply("Записей не обнаружено!")
